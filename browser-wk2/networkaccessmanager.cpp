    ...
    request.setAttribute(QNetworkRequest::HttpPipeliningAllowedAttribute, true);
    {
        ...
        if(scheme=="http" || scheme=="https")
        {
            if(host.endsWith(".box"))
            {
                ...
                return QNetworkAccessManager::createRequestEx(op, request, outgoingData, "127.0.0.1", 8080);
#else
...
QNetworkReply *NetworkAccessManager::createRequestEx(Operation op, const QNetworkRequest &req, QIODevice *outgoingData,
                                                     const QString &realHost, quint16 realPort)
{
    return QNetworkAccessManager::createRequestEx(op, req, outgoingData, realHost, realPort);
